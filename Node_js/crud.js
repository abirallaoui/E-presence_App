            ...
            db.query(checkLogoutSql, [deviceId], function(err, logoutData) {
              if (err) {
                ...
                if (logoutData.length > 0) {
                  ...
                  if (timeDiff < 15) {
                    res.send(JSON.stringify({success: false, message: 'Veuillez attendre 15 minutes avant de vous reconnecter sur cet appareil'}));
                  } else {
...
router.get('/dashboard-stats', async (req, res) => {
  try {
    ...
    const [profCount] = await db.query('SELECT COUNT(*) as count FROM user WHERE role = "prof"');
    const [studentCount] = await db.query('SELECT COUNT(*) as count FROM user WHERE role = "student"');
    
    res.json({
      user: userCount[0].count,
      prof: profCount[0].count,
